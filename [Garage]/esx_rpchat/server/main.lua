...
RegisterCommand('mechanic', function(source, args, rawCommand)
	...
	if target.job ~= nil and target.job.name == "mechanic" then
		...
		TriggerClientEvent('chat:addMessage', -1, { args = { _U('mechanic_prefix', name), args }, color = { 255, 255, 255 } })

	end
end, false)

RegisterCommand('lidl', function(source, args, rawCommand)
	if source == 0 then
		print('esx_rpchat: you can\'t use this command from rcon!')
		return
	end
	...
	if target.job ~= nil and target.job.name == "lidl" then
		args = table.concat(args, ' ')
		local name = GetPlayerName(source)
		if Config.EnableESXIdentity then name = GetCharacterName(source) end

		TriggerClientEvent('chat:addMessage', -1, { args = { _U('lidl_prefix', name), args }, color = { 255, 255, 255 } })

	end
end, false)

RegisterCommand('ejercito', function(source, args, rawCommand)
	if source == 0 then
		print('esx_rpchat: you can\'t use this command from rcon!')
		return
	end
	...
	if target.job ~= nil and target.job.name == "ejercito_de_tierra" then
		args = table.concat(args, ' ')
		local name = GetPlayerName(source)
		if Config.EnableESXIdentity then name = GetCharacterName(source) end

		TriggerClientEvent('chat:addMessage', -1, { args = { _U('ejercito_de_tierra_prefix', name), args }, color = { 255, 255, 255 } })

	end
end, false)

RegisterCommand('armeria', function(source, args, rawCommand)
	if source == 0 then
		print('esx_rpchat: you can\'t use this command from rcon!')
		return
	end
	...
	if target.job ~= nil and target.job.name == "armeria_central" then
		args = table.concat(args, ' ')
		local name = GetPlayerName(source)
		if Config.EnableESXIdentity then name = GetCharacterName(source) end

		TriggerClientEvent('chat:addMessage', -1, { args = { _U('armeria_central_prefix', name), args }, color = { 255, 255, 255 } })
