...
LoadModel = function(model)
    ...
    if HasModelLoaded(model) then
        ...
        if not IsModelInCdimage(model) then
            exports['okokNotify']:Alert("ERROR", "Model " .. ogmodel .. " no está en la imagen del CD", 8000, 'error')
        else
            exports['okokNotify']:Alert("INFORMACIÓN", "Póngase en contacto con el propietario del servidor, el modelo no se pudo cargar (¿no existe?): " .. ogmodel, 10000, 'info')
        end
                        ...
                        if data2.current.value == "items" then
                            ESX.UI.Menu.Open("dialog", GetCurrentResourceName(), "amount_deposit_withdraw'", {
                                ...
                                if amount ~= nil then
                                    ...
                                    exports['okokNotify']:Alert("INFORMACIÓN", Strings["must_be_number"], 8000, 'info')
                                end
                            ...
                            if current2 == "deposit" then
                                if current == "cash" then
                                    if inventory.cash >= amount then
                                        ...
                                        exports['okokNotify']:Alert("ERROR", Strings["dont_have"], 8000, 'error')
                                    end
                                else
                                ...
                                if current == "cash" then
                                    if v.storage.cash >= amount then
                                        ...
                                        exports['okokNotify']:Alert("ERROR", Strings["not_enough"], 8000, 'error')
                                    end
                                else
                ...
                if player ~= "none" then
                    ESX.UI.Menu.Open("default", GetCurrentResourceName(), "confirm_transfer", {
                        ...
                        if #(GetEntityCoords(PlayerPedId()) - GetEntityCoords(data2.current.ped)) <= 5.0 then
                            ...
                            exports['okokNotify']:Alert("ERROR", Strings["nolonger_nearby"], 8000, 'error')
                        end
                    end, function(data3, menu3)
                        menu3.close()
                    end)
...
AddEventHandler("loaf_housing:alertPolice", function(houseInformation)
    if ESX.GetPlayerData() and ESX.GetPlayerData().job then
        if ESX.GetPlayerData().job.name == Config.LockPicking.Police then
            exports['okokNotify']:Alert("INFORMACIÓN", Strings["robbery_progress"], 8000, 'info')
