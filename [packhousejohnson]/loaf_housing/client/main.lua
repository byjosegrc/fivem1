    ...
    ESX.TriggerServerCallback("loaf_housing:spawnHouse", function(success) 
        ...
        if success then
                                ...
                                if #(selfCoords - real_word_position) <= 2.0 then
                                    ...
                                    if IsControlJustReleased(0, ((Config.Options and Config.Options.SecondaryAction) or (47))) then
                                        ...
                                        if (closest == -1 or closest == PlayerId()) or dist > 3.5 then
                                            ...
                                            exports['okokNotify']:Alert("ERROR", Strings["nearby_storage"] or "alguien está cerca, no puede acceder al almacenamiento", 8000, 'error')
                                        end
                                    end
                                end
                            end
            ...
            elseif success == "locked" then
                exports['okokNotify']:Alert("ERROR", Strings["error_locked"], 8000, 'error')
            end
        else
            exports['okokNotify']:Alert("ERROR", Strings["no_spawn"], 8000, 'error')
        end
