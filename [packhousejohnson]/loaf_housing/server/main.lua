...
CreateThread(function()
                    ...
                    MySQL.Async.fetchScalar("SELECT `housedata` FROM `loaf_housing` WHERE `identifier` = @identifier", {
                        ...
                        if currentHouseData then
                            ...
                            if owned_houses >= Config.MaxProperties then
                                TriggerClientEvent('okokNotify:Alert', xPlayer.source, "ERROR", Strings["cant_own_more"], 8000, 'error')
                                toReturn = false
                                return 
                            end
                        end
                                        ...
                                        if furniture[furnitureid].storage then
                                            ...
                                            if storage_data.black_money == 0 and storage_data.cash == 0 then
                                                ...
                                                if amount_items_or_weapons > 0 then
                                                    can_remove = false
                                                    TriggerClientEvent('okokNotify:Alert', xPlayer.source, "ERROR", Strings["has_items"], 8000, 'error')
                                                end
                                            else
                                                can_remove = false
                                                TriggerClientEvent('okokNotify:Alert', xPlayer.source, "ERROR", Strings["has_items"], 8000, 'error')
    ...
    AddEventHandler("loaf_housing:toggleLocked", function(houseid)
        ...
        if spawned_houses[tostring(houseid)] then
            if unlocked_doors[tostring(houseid)] then
                ...
                TriggerClientEvent('okokNotify:Alert', source, "COMPLETADO", Strings["locked_door"], 8000, 'success')
                Log(src, "Lock", "Locked house #" .. houseid)
            else
                unlocked_doors[tostring(houseid)] = true
                TriggerClientEvent('okokNotify:Alert', source, "COMPLETADO", Strings["unlocked_door"], 8000, 'success')
    ...
    AddEventHandler("loaf_housing:sellHouse", function(houseid)
        ...
        if not spawned_houses[houseid] then
            ...
            TriggerClientEvent('okokNotify:Alert', xPlayer.source, "ERROR", Strings["someone_in_house_cant_sell"], 8000, 'error')
        end
    end)
        ...
        if xPlayer and xPlayer.identifier and houseid and type(houseid) == "string" and Config.LockPicking and Config.LockPicking.Enabled then
            if Config.LockPicking.MinimumPolice then
                if GetCops() < Config.LockPicking.MinimumPolice then
                    TriggerClientEvent('okokNotify:Alert', xPlayer.source, "ERROR", Strings["not_enough_cops"], 8000, 'error')
                    return
                end
            end
            ...
            if houseInformation then
                if #(houseInformation.Entrance.xyz - xPlayer.getCoords(true)) <= 4.0 then
                    ...
                    TriggerClientEvent('okokNotify:Alert', xPlayer.source, "ERROR", Strings["not_near_door"], 8000, 'error')
                end
            end
        end
    end)
        ...
        if spawned_houses[houseid] then
            for k, v in pairs(spawned_houses[houseid].guests) do
                if v == pid then
                    ...
                    if xPlayer then
                        TriggerClientEvent('okokNotify:Alert', xPlayer.source, "INFORMACIÓN", Strings["you_got_kicked"], 8000, 'info')
                    end
    ...
    ESX.RegisterServerCallback("loaf_housing:transferOwnership", function(src, cb, player, houseid)
        ...
        if spawned_houses[tostring(houseid)] then
            TriggerClientEvent('okokNotify:Alert', xPlayer.source, "ERROR", Strings["house_spawned_transfer"], 8000, 'error')
            cb(false)
